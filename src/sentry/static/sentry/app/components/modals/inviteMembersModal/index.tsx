...
class InviteMembersModal extends AsyncComponent<Props, State> {
  ...
  sendInvite = async (invite: NormalizedInvite) => {
    ...
    this.setState(s => ({
      inviteStatus: {...s.inviteStatus, [invite.email]: {sent: false}},
    }));
    ...
    try {
      ...
      this.setState(s => ({
        inviteStatus: {...s.inviteStatus, [invite.email]: {sent: false, error}},
      }));
      return;
    }

    this.setState(s => ({
      inviteStatus: {...s.inviteStatus, [invite.email]: {sent: true}},
    }));
  };
  ...
  addInviteRow = () =>
    this.setState(s => ({pendingInvites: [...s.pendingInvites, this.inviteTemplate]}));

  setEmails(emails: string[], index: number) {
    this.setState(s => {
      const pendingInvites = [...s.pendingInvites];
  ...
  render() {
    ...
    return (
      <React.Fragment>
        ...
        <AddButton priority="link" onClick={this.addInviteRow} icon="icon-circle-add">
          {t('Add another')}
        </AddButton>
        ...
        <Footer>
          <FooterContent>
            ...
            {complete ? (
              <React.Fragment>
                <Button label="send more" size="small" onClick={this.reset}>
                  {t('Send more invites')}
                </Button>
                <Button label="done" priority="primary" size="small" onClick={closeModal}>
                  {t("I'm done!")}
                </Button>
              </React.Fragment>
            ) : (
